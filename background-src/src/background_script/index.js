...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'RABET_EXTENSION_CONNECT') {
    RABET_EXTENSION_CONNECT(message, sender, sendResponse, sendResponseCollection).then(
      (newWindow) => {
        window = newWindow;
      },
    );
    ...
    RABET_EXTENSION_SIGN(message, sender, sendResponse, sendResponseCollection).then(
      (newWindow) => {
        window = newWindow;
      },
    );
