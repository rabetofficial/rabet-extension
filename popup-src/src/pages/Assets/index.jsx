...
import getAssetWebsite from '../../helpers/horizon/getAssetData';
...
class Assets extends Component {
  ...
  handleDelete({ code, issuer }) {
    ...
    addAssetAction({ code, issuer, limit: '0' }, history.push);
  }

  render() {
    const { match, history } = this.props;
    const { flags, homeDomain } = this.state;

    const { activeAccount } = currentActiveAccount();

    const { balances } = activeAccount;
    const asset = balances.find((x) =>
      x.asset_code === match.params.asset_code
      ...
      <>
        ...
        <div className={classNames('pure-g justify-end', styles.buttons)}>
          ...
          <Button
            ...
            onClick={() => {
              this.handleDelete({ code: asset.asset_code, issuer: asset.asset_issuer });
            }}
          />
        </div>
      </>
